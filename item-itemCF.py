...
neighbours = []
averages = [] #item's average rating for later use
deviations = [] #item's deviation for later use

for i in range(M):
    # ACHAR OS 20 ITEMS MAIS PROXIMOS DE ITEM I
    ...
    for j in range(M):
        ...
        if j != i:
            ...
            common_users = (users_i_set & users_j_set) #interseçao
            ...
            if len(common_users) > limit:
                ...
                dev_j = {user: (rating - avj_j) for user, rating in ratings_j.items()}
...
def predict(i,u):
    ...
    for neg_w, j in neighbours[i]:
        try:
            ...
            #o vizinho pode nao ter sido avaliado o mesmo user
